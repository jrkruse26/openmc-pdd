...
ARG openmc_branch=deploy-clean
ENV OPENMC_REPO='https://github.com/jrkruse26/openmc-pdd'
...
RUN mkdir -p ${HOME}/OpenMC && cd ${HOME}/OpenMC \
    && git clone --shallow-submodules --recurse-submodules --single-branch -b ${openmc_branch} ${OPENMC_REPO} openmc \
    && mkdir build && cd build ; \
